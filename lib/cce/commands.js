...
const SERVICE_START_CONFIRM_MS = 1000;
const SERVICE_STOP_CONFIRM_MS = 1000;
const SERVICE_CONFIRM_POLL_MS = 50;
...
function waitUntil(predicate, timeoutMs) {
  ...
  while (Date.now() <= deadline) {
    ...
    sleepSync(SERVICE_CONFIRM_POLL_MS);
  }
  return false;
}
...
function tailText(filePath, lineCount = SERVICE_LOG_TAIL_LINES) {
  try {
...
function consoleErrorWithLog(message, logPath) {
  console.error(message);
  const logTail = tailText(logPath);
  if (logTail) {
    console.error(`Recent service log (${logPath}):\n${logTail}`);
  } else {
...
export function serviceStart() {
  ...
  try {
    ...
    console.error(`failed to prepare cce service log ${logPath}: ${error instanceof Error ? error.message : String(error)}`);
    return 1;
  }
  let child;
  try {
    child = spawn(process.execPath, [path.join(projectRoot, 'bin', 'cce.js'), 'service', 'run'], {
      cwd: projectRoot,
      detached: true,
    ...
    fs.closeSync(out);
    console.error(`failed to start cce service: ${error instanceof Error ? error.message : String(error)}`);
    return 1;
  }
  child.unref();
  fs.closeSync(out);

  const confirmed = waitUntil(() => {
    const pid = readPidFile(runtime.pidFile);
    return Boolean(pid && isProcessAlive(pid));
  }, SERVICE_START_CONFIRM_MS);
  if (!confirmed) {
    const childStatus = child.pid && isProcessAlive(child.pid) ? `spawned pid ${child.pid} is still running` : 'spawned process exited';
    consoleErrorWithLog(
      `cce service start did not complete within ${SERVICE_START_CONFIRM_MS}ms (${childStatus}).`,
      logPath,
    );
    return 1;
  }
  ...
  const pid = readPidFile(runtime.pidFile);
  console.log(`cce service started; pid=${pid}; log=${logPath}`);
  return 0;
}
...
export function serviceStop() {
  ...
  try {
    ...
    console.error(`failed to stop cce service pid ${pid}: ${error instanceof Error ? error.message : String(error)}`);
    return 1;
  }
  const stopped = waitUntil(() => !fs.existsSync(runtime.pidFile) || !isProcessAlive(pid), SERVICE_STOP_CONFIRM_MS);
  if (!stopped) {
    const logPath = path.join(runtime.logDir, 'service.log');
    consoleErrorWithLog(
      `cce service stop requested, but pid ${pid} is still running after ${SERVICE_STOP_CONFIRM_MS}ms.`,
      logPath,
    );
    return 1;
  }
